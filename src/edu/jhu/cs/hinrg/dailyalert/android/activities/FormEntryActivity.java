...
import edu.jhu.cs.hinrg.dailyalert.android.logic.PropertyManager;
import edu.jhu.cs.hinrg.dailyalert.android.tasks.FormLoaderTask;
import edu.jhu.cs.hinrg.dailyalert.android.tasks.SaveToDiskTask;
import edu.jhu.cs.hinrg.dailyalert.android.utilities.FileUtils;
import edu.jhu.cs.hinrg.dailyalert.android.utilities.GestureDetector;
import edu.jhu.cs.hinrg.dailyalert.android.views.ODKView;
...
public class FormEntryActivity extends Activity implements AnimationListener, FormLoaderListener,
    ...
    public static final int INSTANCE_UPLOADER = 10;
    ...
    public static final String KEY_FORMID = "keyid";
...
//        mGestureDetector = new GestureDetector();
        ...
        if (savedInstanceState != null) {
            ...
            if (savedInstanceState.containsKey(KEY_FORMID)) {
                mFormId = savedInstanceState.getLong(KEY_FORMID);
            }
            ...
            if (savedInstanceState.containsKey(SERVICE_CALL)){
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putLong(KEY_FORMID, mFormId);
    ...
    private void deleteInstanceFiles() {
...
//                    Log.d(TAG, answers.get(index).getDisplayText());
...
//                }
                ...
                nextButton = (Button) startView.findViewById(R.id.enterButton);
                nextButton.setWidth(GlobalApp.BUTTON_WIDTH);
            ...
            case FormEntryController.EVENT_END_OF_FORM:
                ...
                ((Button)endView.findViewById(R.id.save_exit_button)).setWidth(GlobalApp.BUTTON_WIDTH);
                prevButton = (Button) endView.findViewById(R.id.prevButton);
                prevButton.setWidth(GlobalApp.BUTTON_WIDTH);
                ...
                uploadCheckBox = ((CheckBox) endView.findViewById(R.id.directly_upload));
                ...
                ((Button) endView.findViewById(R.id.save_exit_button))
                        .setOnClickListener(new OnClickListener() {
                            ...
                            public void onClick(View v) {
                            	...
                            	upload = uploadCheckBox.isChecked();
//                            	Log.v(TAG,InstancePath);
                            	saveDataToDisk(EXIT, mInstanceComplete.isChecked());
                            }
                        });
                
//                FormEntryPrompt p = mFormController.getQuestionPrompt();
//                Log.i(TAG, p.getAnswerText());
                
                
//                for( FormEntryPrompt p : mFormController.getQuestionPrompts()){
//                	Log.i(TAG, p.getAnswerText());
//                }
            ...
            case FormEntryController.EVENT_GROUP:
                ...
                odkv.addChildView(buttonLayout, mLayout);
...
//    }
    ...
    private void createQuitDialog() {
        String[] items =
            {
        		getString(R.string.do_not_save), getString(R.string.keep_changes)            };
...
//        DailyAlertService.formFinishIndex++;
        mFormController = null;
        InstancePath = null;
        super.onDestroy();
    ...
    public void loadingComplete(FormController fc) {
        ...
        if (InstancePath == null) {
            ...
            String time =
                app.timeString(new Date());
            String file =
                mFormPath.substring(mFormPath.lastIndexOf('/') + 1, mFormPath.lastIndexOf('.'));
            String phoneID = app.getStringPhoneID();
            String path = FileUtils.INSTANCES_PATH + file;
            if (FileUtils.createFolder(path)) {
                InstancePath = path + "/" + GlobalApp.PREFIX + "_" + GlobalApp.FORM_PREFIX + "_" 
                		+ file + "_" + userID + "_" + phoneID + "_" + time + ".xml";
            }
        } else {
     ...
     * (non-Javadoc)
     * 
     * @see edu.jhu.cs.hinrg.dailyalert.android.listeners.FormLoaderListener#loadingError(java.lang.String)
     * 
     ...
     * (non-Javadoc)
     * 
     * @see edu.jhu.cs.hinrg.dailyalert.android.listeners.FormSavedListener#savingComplete(int)
     * 
    ...
    public void savingComplete(int saveStatus) {
        ...
        switch (saveStatus) {
            ...
            case SaveToDiskTask.SAVED_AND_EXIT:
                ...
                finish();
                break;
            case SaveToDiskTask.SAVED_AND_UPLOAD:
                Toast.makeText(getApplicationContext(), getString(R.string.data_saved_ok),
                    Toast.LENGTH_SHORT).show();
    			...
    			String aesFilename = rootPath + "/"+ GlobalApp.FORM_SUBDIR +"/" +
    					filename.substring(filename.lastIndexOf("/")+1) 
    					+ "." + GlobalApp.AES_FILE_EXTENSION;
    			app.encryptFile(TAG, filename, aesFilename);
                // if it's online, start upload
                if(this.isOnline())
                {	
                	uploadInstance();
	             	// delete the triggered form in db
	                FileDbAdapter fda = new FileDbAdapter();
	                fda.open();
	                Log.d(TAG,"id="+mFormId);
	                fda.deleteFile(mFormId);
	                fda.close();
                }
                // if it's offline, upload later.
                else{
                	Toast.makeText(getApplicationContext(), getString(R.string.data_saved_postponed),
    ...
    private void uploadInstance() {
		// upload all completed forms
    	// bundle intent with upload files
    	ArrayList<String> list = getCompletedFormList();
    	list.add(InstancePath);
    	Log.i(TAG, "current " + InstancePath);
    	
        Intent i = new Intent(this, InstanceUploaderActivity.class);
        
       
        i.putExtra(FormEntryActivity.KEY_INSTANCES, list);
        startActivityForResult(i, INSTANCE_UPLOADER);
	}

    private ArrayList<String> getCompletedFormList()
    {
    	FileDbAdapter fda = new FileDbAdapter();
        fda.open();
        ...
        {
        	String form = c.getString(c.getColumnIndex(FileDbAdapter.KEY_FILEPATH));
        	Log.i(TAG, "form " + form);
        	formlist.add(form);
        	c.moveToNext();
        }
